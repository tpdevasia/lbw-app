...
shinyServer(function(input, output) {
  ...
  output$plot <- renderPlot({
      ...
      data_ranks$colorBuckets <- as.numeric(cut(data_ranks$rank_est, 10))
      ...
      legend("right", leg.txt, horiz = FALSE, fill = colors, bty="o", cex=1, title="Decile of Ranks")
  ...
  output$table_top <- renderTable({
      ...
      # Subset data frame 
      rt <- data.frame(data_ranks$state, data_ranks$county, data_ranks$lbw_births, data_ranks$births, data_ranks$est_samp, data_ranks$lb_samp, data_ranks$ub_samp, data_ranks$rank_est, data_ranks$lb_est, data_ranks$ub_est)
      
      # Assign relevant column names
      colnames(rt) <- c("State", "County", "LBW births", "Total births", "Estimated LBW births/1000 births", "Lower Bound of Estimate", "Upper Bound of Estimate", "Rank", "Lower Bound of Rank", "Upper Bound of Rank")
      rt <- rt[order(rt$Rank), ]
  ...
  output$table_bottom <- renderTable({
      ...
      bottom <- bottom[order(bottom$Rank, decreasing=TRUE), ]
      bottom
  })
  ...
  output$ci_top <- renderPlot({
      ...
      ranks_ci <- data_ranks
      ranks_ci <- ranks_ci[order(ranks_ci$rank_est), ]
      ranks_ci$stcnty <- factor(ranks_ci$stcnty, levels = ranks_ci$stcnty)
    
      # Create confidence interval plot
      p_top <- ggplot(ranks_ci[1:input$slider,], aes(stcnty, rank_est)) + geom_point()
      p_top <- p_top + theme(axis.text.x = element_text(angle = 90)) + geom_pointrange(aes(ymax = ub_est, ymin = lb_est), coloe="black") + scale_y_reverse(lim=c(max(ranks_ci$rank_est)+10,0))
      p_top 
  })
  ...
  output$ci_bottom <- renderPlot({
      ...
      # Create confidence interval plot
      n <- length(ranks_ci$rank_est)
      p_bottom <- ggplot(ranks_ci[(n-input$slider+1):n,], aes(stcnty, rank_est)) + geom_point()
      p_bottom <- p_bottom + theme(axis.text.x = element_text(angle = 90)) + geom_pointrange(aes(ymax = ub_est, ymin = lb_est), color="black") + scale_y_reverse(lim=c(max(ranks_ci$rank_est)+10,0))
      p_bottom 
  })
